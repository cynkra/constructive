...
## Implement new constructors or new methods in your package

To define a new constructor for a class that is already handled:

* Import the necessary functions from constructive
* Define a method `.cstr_construct.<class>.<constructor>`
* Register it, e.g. with roxygen2 by using `#' @export` on top and running
  `devtools::document()`

To handle a new class:

* Import the necessary functions from constructive
* Define a method `.cstr_construct.<class>`
* Register it, e.g. with roxygen2 by using `#' @export` on top and running
  `devtools::document()`
* Define constructors for it, as explained above,
  but with the additional roxygen2 line: `#' @method .cstr_construct.<class> <constructor>`

The package {constructive.examples} accessible at https://github.com/cynkra/constructive.example/
contains a few examples and might be used as templates.
...
Raise issues if anything is unclear.

If this is not enough, the sections below explain how the package works.
...
* We finally dispatch to a method based on the constructor, and not on the object's
  class as is done traditonally. This explains the unusual look of the `UseMethod()`
  call above.
...
Their arguments are `x` and `...`, and no more. Additional parameters fed
...
another constructor, it happens above because Infinite, NA, or decimal dates
...
That last line of the function is essential, it does the attribute reparation.
