...
to use them with the argument `commented = TRUE` and take a look at examples
in the package itself (scripts starting with `s3` and `s4`) or in the 
package {constructive.examples} accessible at 
https://github.com/cynkra/constructive.example/ and designed just for that.

If this is not enough, or if you just want to know more, we describe below in 
